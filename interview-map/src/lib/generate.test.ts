...
describe('generateQuestion', () => {
  ...
  it('returns skip', async () => {
    ...
    expect(await generateQuestion('n', 't', 4)).toEqual({ ok: true, skip: true })
  })
  it('maps 429', async () => {
    invoke.mockResolvedValue({ data: null, error: { context: { status: 429 } } })
    expect(await generateQuestion('n', 't', 1)).toEqual({ ok: false, reason: 'rate_limited' })
  })
  it('maps 401 to unauthenticated', async () => {
    invoke.mockResolvedValue({ data: null, error: { context: { status: 401 } } })
    expect(await generateQuestion('n', 't', 1)).toEqual({ ok: false, reason: 'unauthenticated' })
  })
  it('maps a generic error to gen_error', async () => {
    invoke.mockResolvedValue({ data: null, error: { context: { status: 500 } } })
    expect(await generateQuestion('n', 't', 1)).toEqual({ ok: false, reason: 'gen_error' })
  })
  ...
  it('network reason when invoke throws', async () => {
    ...
    expect(await generateQuestion('n', 't', 1)).toEqual({ ok: false, reason: 'network' })
  })
})
