...
**꼬리 Q1-1. "VDOM diff가 실제 DOM 조작보다 항상 빠른가요?"**
> 아닙니다. VDOM 자체도 JS 객체 트리를 만들고 비교하는 비용이 듭니다. 손으로 최적화한 직접 DOM 조작이 더 빠를 수도 있어요. VDOM의 진짜 가치는 "선언적으로 UI를 짜도 프레임워크가 변경분만 반영해준다"는 개발 편의·일관성이지, 물리적 최고 속도가 아닙니다.

**꼬리 Q1-2. "그 diff를 효율적으로 하려고 React가 두는 가정은 뭔가요?"**
> 두 휴리스틱입니다. ① 타입이 다른 엘리먼트는 서로 다른 트리로 보고 통째로 교체(같은 위치의 div↔span이면 재사용 안 함). ② 리스트의 자식은 key로 동일성을 판단해, key가 안정적이면 이동/유지를 알아보고 재사용합니다. 그래서 key를 인덱스로 주면 순서가 바뀔 때 오판이 생깁니다.
...
**꼬리 Q2-1. "React.memo를 씌웠는데도 자식이 계속 리렌더되면 뭘 의심하나요?"**
> 대개 props로 넘기는 값이 매 렌더마다 새로 만들어지는 참조형입니다. 인라인 객체·배열·함수(`onClick={() => ...}`)는 렌더마다 새 참조라 memo의 얕은 비교가 항상 "달라졌다"로 판정합니다. 함수는 useCallback, 객체/배열은 useMemo로 참조를 고정해야 memo가 실제로 스킵합니다.

**꼬리 Q2-2. "그럼 모든 함수·값에 useCallback·useMemo를 붙이면 되나요?"**
> 아닙니다. 각 훅도 의존성 배열 비교와 캐시 저장 비용이 있고, 대부분의 컴포넌트는 리렌더가 충분히 싸서 오히려 코드만 복잡해집니다. 프로파일러(React DevTools)로 리렌더가 병목이라 확인된 곳, 특히 memo된 자식에 넘기는 값에만 선별 적용하는 게 원칙입니다.
