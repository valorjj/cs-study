...
- [J6. 태스크 큐 vs 마이크로태스크 큐](#j6-태스크-큐-vs-마이크로태스크-큐) — 실행 순서 규칙, await/렌더 타이밍, starvation
- [J7. this 바인딩 규칙](#j7-this-바인딩-규칙) — 4대 규칙 + 우선순위, 화살표 함수, 흔한 유실 사고
...
**꼬리 Q1-1. "그럼 setTimeout(fn, 0)은 정말 0ms 뒤에 실행되나요?"**
> 아닙니다. 콜백은 즉시 실행이 아니라 태스크 큐에 들어가고, 지금 실행 중인 동기 코드가 콜 스택을 완전히 비운 뒤에야 이벤트 루프가 꺼내 실행합니다. 게다가 명세상 최소 지연(중첩 시 4ms)도 있어, "0ms"는 "가능한 가장 빨리, 단 현재 작업이 끝난 다음"이라는 뜻입니다.

**꼬리 Q1-2. "Promise.then과 setTimeout이 둘 다 대기 중이면 뭐가 먼저인가요?"**
> Promise.then이 먼저입니다. Promise 콜백은 마이크로태스크 큐, setTimeout은 (매크로)태스크 큐에 들어가는데, 이벤트 루프는 매 태스크 사이에 마이크로태스크 큐를 전부 비운 뒤 다음 매크로태스크로 넘어갑니다. 그래서 마이크로태스크가 항상 우선입니다.
...
**꼬리 Q1-1. "반복문에서 var로 클로저를 만들면 왜 값이 다 같게 나오나요?"**
> var는 함수 스코프라 반복문이 도는 동안 변수 하나를 공유합니다. 콜백들은 그 하나의 변수를 참조하는 클로저가 되고, 실제 실행 시점엔 반복이 이미 끝나 최종값(예: length)만 남아 전부 같은 값이 찍힙니다. let은 블록 스코프라 반복마다 새 바인딩이 생겨 각 클로저가 자기 값을 캡처합니다.

**꼬리 Q1-2. "클로저가 메모리 누수로 이어질 수 있나요?"**
> 있습니다. 클로저가 참조하는 외부 변수는 클로저가 살아있는 한 GC 대상이 안 됩니다. 이벤트 리스너·타이머 콜백이 큰 객체나 DOM 노드를 캡처한 채 해제되지 않으면 그 메모리가 계속 잡혀 누수가 됩니다. removeEventListener나 참조 해제로 관리해야 합니다.
...
# J6. 태스크 큐 vs 마이크로태스크 큐

**학습 목표**: *"setTimeout(0)과 Promise.then 중 뭐가 먼저인가요?"* / *"마이크로태스크 큐는 언제 비워지나요?"* / *"await 다음 코드는 언제 실행되나요?"* / *"마이크로태스크가 렌더링을 막을 수 있나요?"* 에 실행 순서를 짚으며 5분 답할 수 있다.
> J1에서 이벤트 루프를 개괄했다면, 여기서는 **두 큐의 우선순위 규칙과 실행 순서 예제**를 집중적으로 파고든다.

## 1. 비유 — 은행 창구의 VIP 줄과 일반 줄

창구 직원(콜 스택)은 한 명. **VIP 줄(마이크로태스크: Promise.then, queueMicrotask, MutationObserver)** 과 **일반 줄(매크로태스크: setTimeout, setInterval, I/O, UI 이벤트)** 이 따로 있다. 규칙: 지금 손님을 다 처리하면(콜 스택이 비면) **VIP 줄을 완전히 다 비운 뒤에야** 일반 줄에서 딱 한 명을 부른다. 그 한 명을 처리하는 동안 VIP가 또 오면, 그 일반 손님이 끝난 직후 다시 VIP 줄부터 싹 비운다.
...
Promise.resolve()
  ...
  .then(() => console.log('4'));               // 마이크로(3 이후 추가)
console.log('5');                              // 동기
// 출력: 1 5 3 4 2
// 동기(1,5) → 마이크로 전부(3, 그리고 그 안에서 생긴 4) → 매크로(2)
```
- `.then` 체인은 **한 번에 하나씩** 마이크로태스크로 예약된다 → 3이 실행되면서 4가 큐에 추가되고, 같은 턴에 4까지 비운 뒤 매크로 2로 넘어감.

## 4. await와의 관계

```js
async function f() {
  console.log('A');           // 동기 (호출 시 즉시)
  await something();          // something()은 동기 실행, 그 "이후"가 마이크로태스크
  console.log('B');           // ← 마이크로태스크로 스케줄
}
f(); console.log('C');
// 출력: A C B  — await 지점에서 함수가 "양보", 이후 코드는 마이크로태스크
```
- `await x` = "x를 기다린 뒤의 코드를 `.then` 콜백으로 등록"과 동치. 그래서 **await 이후 코드는 항상 현재 동기 코드가 끝난 뒤(마이크로태스크)** 실행.

## 5. 핵심 포인트 (자주 하는 실수)

- 🔴 "setTimeout(fn, 0)은 즉시 실행" ❌ — 최소 지연 후 **매크로태스크 큐에 줄서기**. 동기 코드 + 모든 마이크로태스크가 끝나야 실행.
- 🔴 "Promise.then이 동기 코드보다 먼저" ❌ — **동기 코드가 항상 우선**. 마이크로태스크는 "동기 이후, 다음 매크로 이전".
- 🔴 마이크로태스크에서 무한히 마이크로태스크를 추가하면 **매크로태스크(렌더 포함)가 영원히 밀림 → starvation**(브라우저 멈춤). 무거운 반복은 setTimeout 등 매크로로 쪼개 렌더링에 양보.
- 🟡 렌더링(리페인트)은 매크로태스크 사이에 일어남 — 마이크로태스크 큐가 계속 차 있으면 화면 갱신이 지연됨.

## 6. 예상 면접 질문 + 답변 골격

**Q1. "setTimeout(0)과 Promise.then 중 뭐가 먼저 실행되나요?"**
> ① 둘 다 비동기지만 큐가 다름 → ② 콜 스택이 비면 마이크로태스크(Promise.then)를 전부 비운 뒤 매크로태스크(setTimeout)를 하나 꺼냄 → ③ 따라서 Promise.then이 먼저. setTimeout(0)도 "즉시"가 아니라 매크로 큐 줄서기.

**Q2. "마이크로태스크 큐는 언제 비워지나요?"**
> ① 콜 스택이 빌 때마다(동기 코드가 끝날 때) → ② 매크로태스크 하나가 끝날 때마다 → ③ 그때 큐가 "완전히 빌 때까지" 전부 실행(실행 중 추가된 것도 이번 턴에 처리).

**Q3. "await 다음 코드는 언제 실행되나요?"**
> ① await는 그 지점에서 함수 실행을 양보 → ② 이후 코드는 `.then` 콜백처럼 마이크로태스크로 스케줄 → ③ 현재 동기 코드가 끝난 뒤 마이크로태스크 단계에서 실행(그래서 `A C B` 순서).

**Q4. "마이크로태스크가 렌더링을 막을 수 있나요?"**
> ① 렌더링은 매크로태스크 사이에 일어남 → ② 마이크로태스크 큐가 계속 채워지면 그 사이로 못 넘어가 렌더가 밀림(starvation) → ③ 무거운 반복 작업은 매크로태스크로 쪼개 브라우저에 양보해야 함.

---

# J7. this 바인딩 규칙

**학습 목표**: *"this는 어떻게 결정되나요?"* / *"call/apply/bind 차이는?"* / *"화살표 함수의 this는 뭐가 다른가요?"* / *"메서드를 콜백으로 넘기면 왜 this가 사라지나요?"* 에 규칙 우선순위로 5분 답할 수 있다.
> J3에서 프로토타입/this를 개괄했다면, 여기서는 **this 결정 4대 규칙의 우선순위와 실전 유실 사고**를 파고든다.

## 1. 비유 — "나"가 누구냐는 말하는 상황이 정한다

`this`는 함수를 **어떻게 선언했느냐가 아니라 어떻게 호출했느냐**로 정해진다. "저 좀 도와주세요"에서 '저'가 누구인지는 말한 사람이 아니라 **그 말이 불린 상황**이 정하는 것과 같다. 그래서 같은 함수라도 호출 방식에 따라 this가 매번 달라진다(화살표 함수만 예외 — 미리 정해져 안 바뀜).

## 2. 4대 규칙 (우선순위 높은 순)

| 우선순위 | 규칙 | this | 예 |
|:---:|------|------|----|
| 1 | **new 바인딩** | 새로 생성된 객체 | `new Foo()` → this = 새 인스턴스 |
| 2 | **명시적 바인딩** | 지정한 객체 | `fn.call(obj)`, `fn.apply(obj)`, `fn.bind(obj)` |
| 3 | **암시적 바인딩** | 점 앞의 객체 | `obj.method()` → this = obj |
| 4 | **기본 바인딩** | (strict) undefined / (비strict) 전역 | `fn()` 단독 호출 |

> 화살표 함수는 이 규칙을 따르지 않고 **선언된 위치의 상위 스코프 this를 그대로 캡처(lexical this)** — call/apply/bind로도 못 바꾼다.
...
greet.call(user, 'Hi');          // 즉시 호출, 인자 나열 → "Hi, Kim"
greet.apply(user, ['Hi']);       // 즉시 호출, 인자 배열   → "Hi, Kim"
const bound = greet.bind(user);  // 호출 안 함, this 고정된 새 함수 반환
bound('Hi');                     // "Hi, Kim"
```
- **call vs apply**: 인자 전달 방식만 다름(나열 vs 배열). **bind**: 즉시 실행하지 않고 this가 묶인 **새 함수**를 반환.

## 4. 흔한 유실 사고 — 메서드를 떼어내면
...
obj.hello();                     // "Kim" (암시적 바인딩)

const fn = obj.hello;
fn();                            // undefined! (점이 사라져 기본 바인딩)

setTimeout(obj.hello, 100);      // undefined! (콜백으로 넘기며 분리됨)

// ✅ 해결
setTimeout(() => obj.hello(), 100);   // 화살표로 감싸 호출 형태 유지
setTimeout(obj.hello.bind(obj), 100); // bind로 this 고정
```
- 원인: this는 **호출 순간의 점(.) 앞 객체**로 정해지는데, 변수에 담거나 콜백으로 넘기면 그 연결이 끊긴다.
- 화살표 함수는 lexical this라 이 문제가 없음 → 클래스 메서드를 콜백으로 쓸 때 화살표 필드(`hello = () => {...}`)나 생성자 bind를 씀.

## 5. 핵심 포인트 (자주 하는 실수)

- 🔴 "this는 함수가 선언된 곳으로 정해진다" ❌ — 일반 함수는 **호출 방식**으로 결정(화살표만 선언 위치).
- 🔴 "화살표 함수도 bind로 this를 바꿀 수 있다" ❌ — 화살표는 lexical this라 call/apply/bind가 **무시됨**.
- 🟡 `obj.method`를 변수/콜백으로 넘기면 this 유실 → 화살표 래핑 또는 bind.
- 🟡 화살표 함수를 **객체 메서드로 쓰면** this가 객체가 아니라 상위 스코프(보통 전역)를 가리켜 버그 — 객체 메서드는 일반 함수(단축 메서드)로.

## 6. 예상 면접 질문 + 답변 골격

**Q1. "this는 어떻게 결정되나요?"**
> ① 일반 함수의 this는 호출 방식으로 결정 → ② 우선순위: new > 명시적(call/apply/bind) > 암시적(obj.method) > 기본(strict면 undefined) → ③ 화살표 함수만 예외로 선언된 상위 스코프의 this를 캡처.

**Q2. "call, apply, bind의 차이는?"**
> ① call=this 지정 후 즉시 호출, 인자 나열 → ② apply=동일하나 인자를 배열로 → ③ bind=즉시 호출하지 않고 this가 고정된 새 함수를 반환(나중에 호출).

**Q3. "화살표 함수의 this는 뭐가 다른가요?"**
> ① 일반 함수는 호출 시점에 this가 정해지지만 → ② 화살표는 선언된 위치의 상위 스코프 this를 캡처(lexical) → ③ call/apply/bind로도 못 바꿈. 그래서 콜백에서 this 유실을 막는 용도로 유용하지만, 객체 메서드로는 부적합.

**Q4. "메서드를 콜백으로 넘기면 왜 this가 사라지나요?"**
> ① this는 호출 순간 점(.) 앞 객체로 정해짐 → ② 변수에 담거나 콜백으로 넘기면 그 연결이 끊겨 기본 바인딩(undefined/전역) → ③ 화살표로 감싸 호출 형태를 유지하거나 bind로 this를 고정해 해결.
