...
## 목차
- **HashMap** — 왜·언제 쓰는가, hashCode→spread→bucket, 충돌 처리(심화)
- **TreeMap · Red-Black Tree** — 정렬 맵, self-balancing, treeify 연결(심화)
- **핵심 질문 (Quiz)** — 플래시카드형 복습
...
<details class="deep">
<summary>심화: Hash Function — key가 bucket index로 변환되는 3단계 (hashCode → spread → &(n-1))</summary>

*"key 하나가 어떻게 bucket index로 변환되는지"* 그리고 *"같은 bucket에 다 몰리면 왜 O(log N)이 되는지"* 코드 한 줄씩 짚어본다.
...
<details class="deep">
<summary>심화: Collision 회피·완화 — 충돌 처리(chaining/treeify)와 equals/hashCode 계약</summary>

*"충돌 어떻게 처리하나요? 어떻게 줄이나요?"* 와 *"equals/hashCode 계약"* 을 코드까지 짚어본다.
...
**학습 목표**: *"정렬이 필요할 때 왜 TreeMap이고"*, *"RB Tree가 어떻게 self-balancing이며 왜 height O(log N)을 보장하는지"*, *"Java HashMap의 treeify와 어떻게 연결되는지"* 5분 답할 수 있다. (5가지 규칙·회전 같은 내부는 아래 **심화**를 펼쳐 확인하세요.)
...
<details class="deep">
<summary>심화: RB Tree 내부 — 5가지 규칙·height 증명·회전·삽입 시나리오</summary>
...
<details class="deep">
<summary>심화: 실전 드릴 복기 — HashMap vs TreeMap 답변 채점 예시</summary>
