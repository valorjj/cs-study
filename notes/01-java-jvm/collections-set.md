...
| `LinkedHashSet` | 명단에 도착 순서대로 줄 세움 | **입장(삽입) 순서 유지** |
...
| 타입 | 내부 구조 | 순서 | 시간복잡도 |
|------|-----------|------|-----------|
| `HashSet` | 요소 → `hashCode` → bucket | 순서 개념 없음 | `O(1)` 평균 |
| `LinkedHashSet` | HashSet + **이중 연결 리스트**로 삽입 순서 기록 | 삽입 순서 | `O(1)` 평균 |
| `TreeSet` | **Red-Black Tree** (키 정렬) | 항상 정렬 상태 | `O(log n)` |

`LinkedHashSet`은 `HashSet`에 연결 리스트를 덧붙인 것이라 **메모리를 조금 더
쓰지만 조회 성능은 같다.** 순서가 필요하면 그냥 이걸 쓰면 된다.
