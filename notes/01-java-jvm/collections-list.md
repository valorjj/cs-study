...
| | 비유 | 임의 접근 | 중간 삽입 |
|---|------|-----------|-----------|
| `ArrayList` | 극장 좌석 (연속 번호) | "12번 좌석" 바로 감 — **`O(1)`** | 뒤 사람 전부 한 칸씩 — `O(n)` |
| `LinkedList` | 보물찾기 쪽지 (다음 위치) | n번째 쪽지까지 따라감 — `O(n)` | 앞뒤 쪽지 2개만 수정 — **`O(1)`** \* |

\* 단, **그 위치를 이미 알고 있을 때**만 `O(1)`이다. 인덱스로 찾아가야 하면
탐색에 `O(n)`이 들어 결국 `O(n)`이 된다 — `LinkedList`의 삽입이 실무에서
기대만큼 빠르지 않은 이유다.
...
**ArrayList** — 연속된 한 덩어리(배열) + 남는 capacity
...
**LinkedList** — 노드가 힙 여기저기 흩어짐

```mermaid
flowchart LR
  A["A<br/><i>{prev, item, next}</i>"] <--> B["B"] <--> C["C"] <--> D["D"]
```

`n`번째 원소 = **순차 추적** → `O(n)`. 게다가 노드마다 캐시 미스가 날 수 있어
같은 `O(n)` 순회도 배열보다 훨씬 느리다.
